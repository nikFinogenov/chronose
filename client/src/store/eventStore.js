	...
	async createEvent(event, selectedCalendar) {
		try {
			...
			if (response.status === 201) {
				this.setEvents(selectedCalendar, [...(this.eventsByCalendar[selectedCalendar] || []), response.data.event]);
	...
	async updateEvent(eventUpdate, calendarId) {
		try {
			...
			if (response.status === 200) {
				this.setEvents(
					calendarId,
					(this.eventsByCalendar[calendarId] || []).map(event => (event.id === eventUpdate.id ? { ...event, ...eventUpdate } : event))
				);
			}
		} catch (error) {
